...
const internals = {
    ...
    isSameMap(m1, m2) {
        ...
        return m1 && m2 && m1.get('uri') === m2.get('uri') && m1.get('byterange') === m2.get('byterange'); // TODO: deep equal
    },
...
const SegmentPointer = class {
    ...
    // @ts-ignore
    static Empty = new SegmentPointer(-1);

    /**
     * @param {number} msn
     * @param {number | undefined} part
     */
    constructor(msn, part = undefined, map = false) {
    ...
    isEmpty() {
        ...
        return this === SegmentPointer.Empty;
    }
};
...
//        console.trace('_checkNext', !!this.readable, !!state.active, !state.fetching, !state.next.isEmpty(), !!index)
        if (!this.readable || !state.active || state.fetching || state.next.isEmpty() || !index) {
            return;
        }
        ...
        if (segment && (this.lowLatency || segment.uri)) {
            ...
            if (next.part !== undefined) {
                ...
                if (active) {
                    ...
                    active.segment.details.parts.push(...segmentParts); // TODO: signal more parts were added
    ...
    async _fetchFrom(ptr, segment) {
        ...
        if (ptr.isMap) {
            ...
            if (segment.map.has('byterange')) {
                byterange = Object.assign({ offset: 0 }, segment.map.get('byterange', AttrList.Types.Byterange));
            }
            else {
                byterange = undefined;
            }
        ...
        try {
            if (uri === undefined || ptr.part) {
                ...
                const parts = segment.parts.slice(ptr.part || 0).map((part) => ({
                    uri: Url.resolve(this.baseUrl, part.get('uri', AttrList.Types.String) || ''),
                    /** @type {Helpers.Byterange | undefined} */
