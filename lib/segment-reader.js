...
internals.fetchFrom = function (reader, seqNo, segment, callback) {
    ...
    return reader.fetcher.fetchUri(Url.resolve(reader.baseUrl, segment.uri), segment.byterange, (err, meta, stream) => {
...
internals.checkNext = function (reader) {
    ...
    if (segment) {
        ...
        state.fetching = internals.fetchFrom(reader, seq, segment, (err, object) => {
            ...
            if (object) {
                if (object.stream) {
                    reader.watch[object.segment.seq] = object.stream;
                    Oncemore(object.stream).once('end', 'error', () => {

                        delete reader.watch[object.segment.seq];
                    });
                }
